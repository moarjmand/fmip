...
import { fetchMe, fetchNotificationSettings } from '@/lib/api';
...
export default async function NotificationSettingsPage({
  ...
  const result = await fetchNotificationSettings(cookie);
  ...
  return (
    <main className="mx-auto flex max-w-2xl flex-col gap-6 p-8">
      ...
      {result.ok ? (
        <NotificationSettingsForm locale={locale} settings={result.data} />
      ) : (
