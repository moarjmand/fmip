...
| 2026-09-20 | Maintainer's laptop, rehearsal as above, on the tree after PR #226 (58 migrations, delivery, the intelligence layer, campaigns, the setup check) | **Partial, and the part that could have regressed passed.** The four images built from this tree; `docker compose run --rm migrate` applied all 58 migrations to an empty database; `postgres`, `redis`, `model`, `api` and `web` all reported healthy -- so nothing the delivery, intelligence or campaigns work added (nodemailer, web-push, the new pages and migrations) breaks the production build or the first start. The edge was **not** re-checked: `caddy` never became healthy, `https://localhost:8443/en` did not answer inside 100 s, and then the Docker daemon itself stopped answering -- `compose ps`, `inspect` and `logs` all hung past their timeouts, with three compose projects up on the host. Nothing in #219-#226 touches the `Caddyfile` or `rollout.sh`, so the zero-downtime claim stands on the 2026-09-19 run above; re-run `verify-rollout.sh` on a quiet machine, or on the VPS, where it belongs. One thing to know before repeating it here: another project on this laptop holds 55432 and 56379, so the rehearsal `.env` needs different `POSTGRES_PORT` and `REDIS_PORT` (55433 and 56380 worked). |
