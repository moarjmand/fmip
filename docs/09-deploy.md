...
# SSH by key only: password guesses start within minutes of the first boot
echo 'PasswordAuthentication no' > /etc/ssh/sshd_config.d/10-fmip-keys-only.conf && sshd -t && systemctl reload ssh
...
**Why keys only.** Ubuntu 24.04 leaves `PasswordAuthentication yes` for every
account but root. No account here has a password, so no guess can succeed, but
the first production server logged 1,675 failed guesses in its first five
hours; with passwords off they are refused before a password is asked for, and
a password set later by mistake cannot open the door. The `10-` prefix puts
the file ahead of any `50-cloud-init.conf` a provider's image adds, and sshd
takes the first value it reads.
