...
**T-082 verified on 2026-09-12 (installability checked mechanically; the
device install is the maintainer's check).**

**Corrected on 2026-09-17: "installs on Android" was true of the build and
false of every image we ship.** `next build` does not put `public` into the
standalone bundle. Both Dockerfiles said so in a comment and copied `static`
only, so in the container -- the preview, and the production compose stack --
`/sw.js` and all three icons answered **404**. The manifest still answered 200,
because it is a route rather than a file, which is exactly why this looked
fine: a valid manifest pointing at icons that are not there. With no service
worker and no icons a browser does not offer to install anything, so the
acceptance criterion could not have held anywhere it mattered.

The E2E pass could not have caught it. `tests/e2e/pwa.spec.ts` runs against
`pnpm start`, which serves `public` from the working tree; the failure exists
only once the app is an image. So the guard went where the mistake is possible:
each image now asserts `public/sw.js` and `public/icons` after copying them,
and a Dockerfile that loses the line again fails to build.

Checked by rebuilding the web image and asking the container: `/sw.js`,
`/icons/icon-192.png`, `/icons/icon-512.png` and `/icons/icon-maskable-512.png`
all answered 404 before and 200 after.

**And then on a phone, 2026-09-17.** With the fix deployed to the preview, the
maintainer opened it in Chrome on Android and the browser's menu offered
**Install app** — Chrome's own judgement that the site is installable, distinct
from "Add to Home screen", which is always there. That is the criterion as
written, met where it can only be met: on a device, on a deployment. The
service worker on that deployment reports `activated` with scope `/`, and all
three icons answer 200 as PNG. `src/app/manifest.ts` serves
`/manifest.webmanifest` (name, short name, `start_url` `/en/scores`,
`standalone`, theme and background colour, 192 and 512 icons and a
full-bleed maskable one, generated by `scripts/make-icons.mjs` into
`public/icons`); the layout links the manifest, the theme colour, the apple
touch icon and the iOS home-screen title; `public/sw.js` is the service
worker (D-042): it caches only the offline page, the manifest, the icons and
Next's immutable build assets, answers navigations from the network and
shows `/[locale]/offline` when there is none, and never serves a cached page
as current (rule 4) — the web app's API routes and streams are never
cached; `components/service-worker.tsx` registers it after load. **Installs
on Android; audited:** Lighthouse 12 no longer carries a PWA category, so
the install criteria are audited in `tests/e2e/pwa.spec.ts` against the
production build: the manifest is served as a manifest with the required
fields and both icon sizes plus a maskable one, every icon answers 200 as
