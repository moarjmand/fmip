...
| `src/modules/catalog/` | The catalog boundary's read side. Today `GET /countries` for the registration form; competition, team and player reads arrive with E3. |
| `src/modules/profile/` | The profile boundary: `GET /profiles/:username` (privacy applied to the viewer before anything is serialised), `GET`/`PATCH /me/profile`, `PATCH /me/privacy`. `internal/visibility.ts` is the one rule (`canView`) plus the `FriendshipOracle` port, whose only implementation answers no until Phase 2. `profile.http.spec.ts` proves public / friends / private server-side. |
