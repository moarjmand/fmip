...
| `src/modules/catalog/` | The catalog boundary's read side. `GET /countries`, `GET /teams` and `GET /competitions` as summary lists for forms and follow controls. T-035: `GET /competitions/:id?season=` — `CatalogService.competition` (season chosen by `pickSeason`: requested, else current, else newest), `internal/competition-store.ts` (competition with country, seasons, stages, every fixture of the season with teams and score, declared coverage); the table and the leaders come from the standings boundary. `competition.http.spec.ts` builds a league with two seasons and proves the selector and the coverage states. Team and player reads follow (T-036, T-037). |
