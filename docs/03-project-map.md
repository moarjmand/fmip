...
| `src/modules/founder/` | The founder's analysis boundary (blueprint 6.5, T-131), separate from the forecast boundary on purpose (rule 6): `founder.controller.ts` is `GET /fixtures/:id/founder-analysis` (public) and `POST` (the `founder` role, not `admin`); `founder.service.ts` turns the database's kick-off refusal (SQLSTATE `PL002`) into a 409; `internal/founder-store.ts` publishes the version and writes its audit row in one transaction; `internal/validate.ts` names every bad field at once. `founder.spec.ts` covers the rules, `founder.http.spec.ts` the gate, the versions, the audit rows and the wall. |
