...
| `packages/ingestion` | The normalised model adapters produce, the adapter contract, and the recorded-fixture harness that verifies an adapter, and the adapters: `api-football` (T-021), `football-data-org` (T-022); `highlightly` arrives with T-023. | `apps/api` |
...
| `src/adapters/<provider>/` *(planned)* | `highlightly` (T-023). |
