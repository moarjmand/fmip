...
| `src/modules/profile/` | The profile boundary: `GET /profiles/:username` (privacy applied to the viewer before anything is serialised), `GET`/`PATCH /me/profile`, `PATCH /me/privacy`. `internal/visibility.ts` is the one rule (`canView`) plus the `FriendshipOracle` port, whose only implementation answers no until Phase 2. `profile.http.spec.ts` proves public / friends / private server-side. `ProfileService.predictionHistoryAccess` (T-056) applies the same rule to `prediction_history_visibility` for the predictions boundary. Following (T-042): `GET`/`PUT`/`DELETE /me/following[/:type/:id]`, `GET /me/favourites`; `internal/following-store.ts` is the SQL; `internal/favourite-order.ts` is `compareByFavourites`, the pure ranking the scores API sorts with (exported from `profile.service.ts`). |
