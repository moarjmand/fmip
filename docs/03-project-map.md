...
| `src/app/[locale]/u/[username]/page.tsx` | A member's profile as the API allows this viewer to see it: full, or name-only when restricted. T-056: the Performance Rating (rating, tier, status, sample, formula version) and the prediction history (`?page=`), each with its own restricted / unreachable states. |
