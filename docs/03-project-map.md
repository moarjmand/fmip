...
| `src/modules/reputation/` | The reputation boundary. T-053: `reputation.service.ts` is public (`ReputationService.current`, `.recompute(userId)` — snapshot only when the inputs changed —, `.recomputeForFixture`, `.recomputeDue`; re-exports the formula); `reputation.controller.ts`: `GET /me/rating`, `GET /users/:username/rating`, `POST /me/rating/recompute`, `POST /ratings/recompute` (admin); `internal/formula.ts` is the versioned formula config and the pure computation (`formula.spec.ts`); `internal/rating-store.ts` the snapshots. `reputation.http.spec.ts` proves recomputability and the favourite-picker penalty end to end. Career Points (T-054) and leaderboards (T-055) follow. |
