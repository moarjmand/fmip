...
| `src/modules/conversations/` | The conversations boundary (blueprint 8.3, Phase 3). T-220 is the schema only: `conversations.schema.spec.ts` covers what the database guarantees -- numbering from one per conversation, a caller-supplied sequence overwritten, a refused message spending no number, `PL006` for a non-participant or somebody who has left, `PL003` across a block, `PL004` under a messaging sanction, and `PL007` on any change to a message but the tombstone. The service and the endpoints are T-221. |
