...
| `src/modules/profile/` | The profile boundary: `GET /profiles/:username` (privacy applied to the viewer before anything is serialised), `GET`/`PATCH /me/profile`, `PATCH /me/privacy`. `internal/visibility.ts` is the one rule (`canView`) plus the `FriendshipOracle` port, whose only implementation answers no until friendships exist (T-201, Phase 3). `profile.http.spec.ts` proves public / friends / private server-side. `ProfileService.predictionHistoryAccess` (T-056) applies the same rule to `prediction_history_visibility` for the predictions boundary. Following (T-042): `GET`/`PUT`/`DELETE /me/following[/:type/:id]`, `GET /me/favourites`; `internal/following-store.ts` is the SQL; `internal/favourite-order.ts` is `compareByFavourites`, the pure ranking the scores API sorts with (exported from `profile.service.ts`). |
| `src/modules/social/` | The social graph boundary (blueprint 8.1, Phase 3). T-200 is the schema only: `social.schema.spec.ts` writes what the service will write and checks the guarantees that belong to the database — the ordered pair, the self-request refusal, `PL003` on a request or a friendship across a block, the block ending the friendship it finds, and a lifted block not restoring it. The service, the controller and the real `FriendshipOracle` are T-201. |
