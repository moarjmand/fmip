...
| `src/modules/forecast/internal/model-client.ts` | The API's side of the internal contract with the model service (T-063): `ModelClient.forecast()` and `.health()`, plus `contractProblems()`, which refuses any response that drifted from `@fmip/contracts`. Failure is a value. The forecast boundary's own tables and service arrive with T-064. |
